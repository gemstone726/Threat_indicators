...
tracks_out = [304339,304462,304648,305691,305701]
tracks_in = [304380]
#tracks_in = [0]
...
def pull_out_important_tracks(tracks_feather, tracks_data):
    ...
    #tracks_feather['True Track ID'] = tracks_feather['True Track ID'].astype(int)
...
def pull_out_important_data(data_feather, real_track_ids_DF):
    ...
    df = data_feather
    #print(data_feather['ID'])

    ##########Clean up later
    x = data_feather.ID.str.split(' ',expand=True,)
    try:
        if len(x) >= 4:
            y = x[4].str.split('\n', expand=True,)[0]
            df['ID']=y
    ...
    for track_ids in db_track_ids:
        df = data_feather[data_feather['ID'] == track_ids]
...
def main():
    ...
    #rint(min_max_mean(important_data))
    #print('========================================================================================================================')
